...
def _spawn_workers(args):
    ...
    Each worker gets a subset of ``args.num_episodes`` and writes into its own
    ``save_dir/proc_<i>`` directory. Seeds are offset by process index so they are
    independent.
    """
    ...
    for i in range(n):
        ...
        sub.num_episodes = cnt
        if sub.seed is not None:
            sub.seed = sub.seed + i
        sub.save_dir = str(pathlib.Path(args.save_dir) / f"proc_{i}")
        p = multiprocessing.Process(target=main, args=(sub,))
        p.start()
        procs.append(p)
    for p in procs:
        p.join()
    Logger.log_info(f"Finished {n} worker processes; subdirectories created under {args.save_dir}.")
...
if __name__ == "__main__":
    ...
    # make multiprocessing use spawn (safer with CUDA)
    try:
